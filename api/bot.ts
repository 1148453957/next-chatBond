...
http.interceptors.request.use((config) => {
  ...
  if (config.withoutBaseInfo) {
    ...
    config.data = {
      appId,
      baseInfo: JSON.parse(cookieStore.get("base_info")?.value!),
      ...data,
    };
  }
  let str = "";
...
export const getUserInfoApi = (params = {}) => {
  return http.post(botDomain + "/api/v1/account/query", params);
};
/**用户界面，新增apikey */
export const addApiKeyApi = (params = {}) => {
  return http.post(botDomain + "/api/v1/account/create/apiKey", params);
};

// 对话导出
export const exportLogsList = (data: any) => {
  return http.post(botDomain + "/api/v1/message/export", data, {
    responseType: "blob",
  });
};
// leads导出
export const exportLeadsList = (data: any) => {
  return http.post(botDomain + "/api/v1/bot/leads/export", data, {
    responseType: "blob",
  });
};
// leads列表
        ...
        reader?.read().then(({ value, done }) => {
          ...
          cb(done, {
            text:
              str.trim() ||
              (done
                ? "The intelligent model is currently busy, please try again later"
                : ""),
            sessionId,
            sessionName,
            messageId,
